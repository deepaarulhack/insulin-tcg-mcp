...
# -----------------------------
# Interactive Orchestrator (HITL)
# -----------------------------
def interactive_pipeline(payload: dict, stage: str = "requirement") -> dict:
    """
    HITL pipeline: waits for user_action ("continue" or "stop") before advancing.
    """
    user_action = payload.get("user_action", "continue")
    ...
    if stage == "jira":
        ...
        state.update({
            ...
            "status": "COMPLETE",
            "message": "✅ Pipeline finished. Do you want to start a new request?"
        })
        return state
